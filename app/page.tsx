...
function PaginaPedidosInner() {
    ...
    async function init() {
      const [{ data: pecasData }, { data: clienteData }] = await Promise.all([
        ...
        clienteId
          ? supabase
              .from('clientes')
              .select('nome_cliente, celular, telefone')
              .eq('id_tiny', clienteId)
              .single()
  ...
  function alterarQuantidade(peca: Peca, delta: number) {
    setCarrinho((prev) => {
      ...
      const atual = novo.get(peca.tiny_id) ?? 0
      const proxima = atual + delta
      if (proxima <= 0) {
        novo.delete(peca.tiny_id)
      } else {
        novo.set(peca.tiny_id, proxima)
      }
      return novo
    })
  }
  ...
  function enviarPedido() {
    ...
    if (itensCarrinho.length === 0) {
      alert('Adicione ao menos uma peça ao pedido.')
      return
    }
    ...
    const linhasItens = itensCarrinho
      .map((item) => {
        ...
        return `• ${item.quantidade}x ${item.nome}${item.codigo ? ` (COD: ${item.codigo})` : ''} - ${formatarPreco(preco)} cada = ${formatarPreco(subtotal)}`
      })
      .join('\n')
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold text-gray-900">Tecnocell</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Coluna principal — catálogo */}
        <div className="lg:col-span-2 space-y-4">
          {/* Dados do cliente — só mostra se NÃO veio pela URL */}
          {!clienteIdentificado && (
                    ...
                    >
                      <div className="flex-1 min-w-0 mr-3">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          {peca.codigo && (
                            <span className="text-xs text-gray-400">COD: {peca.codigo}</span>
                          )}
                          {peca.preco_promocional && parseFloat(peca.preco_promocional) > 0 ? (
                            <span className="flex items-center gap-1">
                              ...
                              <span className="text-xs font-semibold text-green-600">
                                {formatarPreco(parseFloat(peca.preco_promocional))}
                              </span>
                            </span>
                          ) : (
                    ...
                    <div key={item.tiny_id} className="flex justify-between items-start gap-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400">
                          {item.quantidade}x {formatarPreco(preco)}
                        </p>
                      </div>
